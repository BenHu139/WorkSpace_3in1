...
#if(DCM_SERVICE_27_ENABLED == STD_ON)
extern VAR(uint32, DCM_NOINIT_DATA) gSecurityAcessDelayTimeOnBoot[KIND_OF_SECURITY_LEVEL];
extern VAR(uint8, DCM_NOINIT_DATA) gSecurityAcessAttempNumber[KIND_OF_SECURITY_LEVEL];   
/*extern VAR(uint8, DCM_NOINIT_DATA) gSecurityAcessAttempNumberExceeded[KIND_OF_SECURITY_LEVEL];*/
...
#define  UDS_STATUS_ALLOW_TRANS_DATA      (0x01u)
#define  UDS_STATUS_SEGMENT_FIRST_DATA    (0x02u)
#define  UDS_STATUS_ERASE_BLOCK           (0x04u)
#define  UDS_STATUS_CHECK_SUM             (0x08u)

#define  UDS_STATUS_MUL_ALL              (UDS_STATUS_ALLOW_TRANS_DATA\
                                            |UDS_STATUS_SEGMENT_FIRST_DATA\
                                            |UDS_STATUS_ERASE_BLOCK\
                                            |UDS_STATUS_CHECK_SUM)
...
struct _tag_AdapterData
{
    ...
    bl_Size_t checkSize;    /**< the checksum size*/
    bl_Size_t checkAddr;    /**< the checksum addr*/
};

typedef struct _tag_AdapterData bl_AdapterData_t;

extern  bl_AdapterData_t gs_UdsPrivateData;
extern  uint8  uds_download_state;
...
extern FUNC(void,DCM_PUBLIC_CODE) App_CustomizedServcie
(
...
extern void UdsClearDownInfo(bl_AdapterData_t *adpt);

extern bl_Return_t UdsIsValidStatus (uint8 uds_state,uint8 require_state);
extern void UdsClearDownStatus(uint8 *uds_state, uint8 require_state);
extern void UdsSetDownStatus(uint8 *uds_state, uint8 require_state);
