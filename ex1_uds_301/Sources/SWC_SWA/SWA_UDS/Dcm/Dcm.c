...
/*********************wen modify**********************************************/
bl_AdapterData_t gs_UdsPrivateData;
uint8  uds_download_state = 0u;
/*********************wen modify**********************************************/
 ...
 *
 *****************************************************************************/
bl_Return_t UdsIsValidStatus (uint8 uds_state,uint8 require_state)
{
    bl_Return_t ret = BL_ERR_NOT_OK;
    
    if((uds_state & require_state) > 0)
    {
        ret = BL_ERR_OK;
    }
...
/**************************************************************************//**
 *
 *  \details set the download status in the uds_state.
 *
 *  \return None.
 *
 *  \since V2.0.0
 *
 *****************************************************************************/
void UdsSetDownStatus(uint8 *uds_state, uint8 require_state)
{
    *uds_state |= require_state;  
}

/**************************************************************************//**
 *
 *  \details clear the download status in the uds_state.
 *
 *  \return None.
 *
 *  \since V2.0.0
 *
 *****************************************************************************/
void UdsClearDownStatus(uint8 *uds_state, uint8 require_state)
{
    *uds_state &= (~require_state);
}
 ...
 *
 *****************************************************************************/
void UdsClearDownInfo(bl_AdapterData_t *adpt)
{
    adpt->address = 0u;
    adpt->remainSize = 0u;
    adpt->blockIndex = 0u;
    ...
    UdsClearDownStatus(&uds_download_state,UDS_STATUS_MUL_ALL);
    return ;
}
 ...
 *
 *****************************************************************************/
static bl_Return_t ActivateDownload ( uint32 address,uint32 size)
{
...
(
    CONSTP2VAR(Std_VersionInfo_Type,AUTOMATIC,DCM_APPL_DATA) versionInfo
)
{
...
)
{
    ...
    if(SecLevel_Int < KIND_OF_SECURITY_LEVEL)
    {
        gSecLevelForCurrentSecurityAccessRequestLevel = SecLevel_Int;
        result = ((0x01u << SecLevel_Int) & GET_THE_SECURITY_LEVEL);   
    }
    else
    {
        result = DCM_SERVICE_27_SECURITY_MAP_L1; 
    }
...
STATIC FUNC(void,DCM_PRIVATE_CODE) DslInternal_Timer_CallbackTask(void) 
{
    ...
    #if (DCM_SERVICE_27_ENABLED == STD_ON)                           
    if (gSecurityAcessAttempNumber[gCurrentSecurityAccessRequestLevel - 1u] == 0u) 
    {
        if (gSecurityAcessDelayTime[gCurrentSecurityAccessRequestLevel - 1u] != (uint32)DCM_TIME_EXPIRED) 
        {
            gSecurityAcessDelayTime[gCurrentSecurityAccessRequestLevel - 1u]--;
        } 
        else 
        {
            gSecurityAcessAttempNumber[gCurrentSecurityAccessRequestLevel - 1u] = 1u;/*gDcmDspSecurityRow[gCurrentSecurityAccessRequestLevel - 1u].DcmDspSecurityNumAttDelay;*/
            gSecurityAcessDelayTime[gCurrentSecurityAccessRequestLevel - 1u] = gDcmDspSecurityRow[gCurrentSecurityAccessRequestLevel - 1u].DcmDspSecurityDelayTime/DCM_TASK_TIME;
            for (i = 0u; i < KIND_OF_SECURITY_LEVEL; i++) 
            {
                gSecurityAcessSequence[i] = 0u;
            }
    ...
    {
        if (gSecurityAcessDelayTimeOnBoot[DCM_INDEX_0] != (uint32)DCM_TIME_EXPIRED) 
        {
            gSecurityAcessDelayTimeOnBoot[DCM_INDEX_0]--;
        } 
        else 
        {
            gSecurityDelayTimeOnBootFlag = DCM_FLAG_DISACTIVE;
            gSecurityAcessDelayTimeOnBoot[DCM_INDEX_0] = gDcmDspSecurityRow[DCM_INDEX_0].DcmDspSecurityDelayTimeOnBoot/DCM_TASK_TIME;
        }
    } 
    else
    {
...
)
{
    ...
    if((gP2ServerTimer == (uint32)(0 + 1))  ||  (ForceRCR_RP == FORCERCR_RP))  /*0+1  防止由于调度出现的超时问题*/
    {
...
)
{
    ...
    switch(gMsgContextType.reqData[DCM_INDEX_1])
    {
        ...
        #if((reportDTCSnapshotIdentification == STD_ON) \
           || (reportDTCSnapshotRecordByDTCNumber == STD_ON) \
...
)
{
    ...
    /* Check message format */
    if((gMsgContextType.reqDataLen&0x01uL) != (uint32)0u)
    {
        DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
    }
...
)
{
    ...
    addressFormat = 
        (gMsgContextType.reqData[DCM_INDEX_1] & DCM_SERVICE_23_GET_MEMORY_AND_ADDRESS_BYTE_LOW_NIBBLE);
    sizeFormat = 
        (gMsgContextType.reqData[DCM_INDEX_1] >> DCM_SERVICE_23_GET_MEMORY_AND_ADDRESS_BYTE_HIGH_NIBBLE);
    ...
    else
    {
        ...
        for(i = 0u; i < NUMBER_OF_READ_ADDRESS_SEG; i++)
        {
                ...
                && ((gDcmDspReadMemoryRangeInfo_23[addresssegment].AddressingMode - gMsgContextType.msgAddInfo.reqType) < 1u))
                {
...
)
{
    ...
    else if(gSecurityAcessAttempNumber[gCurrentSecurityAccessRequestLevel - 1u] == 0u)
    {
        /*
        if(gSecurityAcessAttempNumberExceeded[gCurrentSecurityAccessRequestLevel - 1u] == 1u)
        {
            DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_EXCEEDNUMBEROFATTEMPTS);
            gSecurityAcessAttempNumberExceeded[gCurrentSecurityAccessRequestLevel - 1u] = 0u;
        }
        else
        {
            DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_REQUIREDTIMEDELAYNOTEXPIRED);
    ...
    /*   Get Seed   */
    else if((gMsgContextType.reqData[DCM_INDEX_1]&1u) != DCM_SERVICE_27_SEND_KEEY) 
    {
        /* Check message format */
        ...
        {               
            ...
            for (i = 0u; i < gDcmDspSecurityRow[gSecLevelForCurrentSecurityAccessRequestLevel - 1u].DcmDspSecuritySeedSize; i++)
            {
            ...
            gSecurityAcessSequence[gCurrentSecurityAccessRequestLevel - 1u] = HAVE_GOTTON_SEED;
            gMsgContextType.resDataLen = 
                (gDcmDspSecurityRow[gCurrentSecurityAccessRequestLevel - 1u].DcmDspSecuritySeedSize + (uint32)DCM_SERVICE_AND_SUB_SERVICE_RESPONSE_LENGTH);    
            DsdInternal_ProcessingDone(&gMsgContextType);
        }
        ...
        {
            gCurrentSecurityAccessRequestLevel = gSecLevelForCurrentSecurityAccessRequestLevel;
            gSecurityAcessSequence[gSecLevelForCurrentSecurityAccessRequestLevel - 1u] = HAVE_GOTTON_SEED;
            gMsgContextType.resDataLen = 
                (gDcmDspSecurityRow[gCurrentSecurityAccessRequestLevel - 1u].DcmDspSecuritySeedSize + (uint32)DCM_SERVICE_AND_SUB_SERVICE_RESPONSE_LENGTH); 
    ...
    else
    {
        ...
        if (gMsgContextType.reqDataLen != 
            (gDcmDspSecurityRow[gSecLevelForCurrentSecurityAccessRequestLevel-1].DcmDspSecurityKeySize + (uint32)DCM_SERVICE_AND_SUB_SERVICE_REQUIRE_LENGTH))
        {
            DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
        }
        /**/        
        ...
        {   
            ...
            else if(gCurrentSecurityAccessRequestLevel != gSecLevelForCurrentSecurityAccessRequestLevel)
            {
                DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_REQUESTSEQUENCEERROR);
            }
            else
            ...
            switch(gMsgContextType.reqData[DCM_INDEX_3])
            {
                case RETURN_CONTROL_TO_ECU:
                {
                    ...
                    if(gMsgContextType.reqDataLen != ((uint32)DCM_SERVICE_2F_RETURN_CONTROL_TO_ECU_REQ_LENGTH\
                        + gDcmDsdSubService_2F[didposition].RecordSizes.DcmDspDidControlEnableMaskRecordSize))
                    {
                        DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
                    }
                    else
                ...
                case SHORT_TERM_ADJUSTMENT:
                {
                    ...
                    if(gMsgContextType.reqDataLen != ((uint32)DCM_REQUIRE_DATA_LENGTH_4_BYTE 
                        + gDcmDsdSubService_2F[didposition].RecordSizes.DcmDspDidControlEnableMaskRecordSize
                        + gDcmDsdSubService_2F[didposition].RecordSizes.DcmDspDidControlStatusRecordSize))
                    {
                        DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT);
                    }
                    else
...
/* BEGIN_FUNCTION_HDR
********************************************************************************
* Function Name: DspInternal_Uds_RequestDowndload
*                
* Description:   Implementation of Uds servie 0x34
*                             
* Inputs:        None
*                
* Outputs:       None
...
FUNC(void,DCM_PUBLIC_CODE) DspInternal_Uds_RequestDowndload
(
    void
)
{
     ...
     if(1u == REMOTEUPDATE_GetEraseMemoryResult())
     {
         do
         {        
             ret = UdsIsValidStatus(uds_download_state, UDS_STATUS_ALLOW_TRANS_DATA);
             if(BL_ERR_OK == ret)
             {
             ...
             if((ADPT_DATAFORMAT_IDEN != gMsgContextType.reqData[ADPT_UDS_34_SERVICE_IDEN_POS])
                 || (ADPT_MEMORY_LENANDADDR_FORMAT
                         !=  gMsgContextType.reqData[ADPT_UDS_34_SERVICE_ADDRANDSIZE_POS]))
             {
                 resCode = DCM_E_CONDITIONSNOTCORRECT;
                 break;
             }

             addr = BL_BE32_TO_MCU(&gMsgContextType.reqData[ADPT_UDS_34_SERVICE_ADDRESS_POS]);
             reqSize = BL_BE32_TO_MCU(&gMsgContextType.reqData[ADPT_UDS_34_SERVICE_SIZE_POS]);

             ret = ActivateDownload(addr, reqSize);
             if(BL_ERR_OK == ret)
             {
                 gMsgContextType.resData[ADPT_UDS_34_RESPONSE_FORMAT_POS] = ADPT_LENFORMAT_IDEN;
                 ...
                 BL_MCU_TO_BE16(&gMsgContextType.resData[ADPT_UDS_34_RESPONSE_SIZE_POS],
     ...
     #error "ADPT_IDEN_LEN_FORMAT is error."
                 ...
                 gs_UdsPrivateData.checkSize = reqSize;
                 gs_UdsPrivateData.checkAddr = addr;
                 gs_UdsPrivateData.blockIndex = 0x01u;
                 
                 REMOTEUPDATE_Init();

                 gMsgContextType.resDataLen = ADPT_REQDOWNLOAD_RESPLENGTH + 1;

                 UdsSetDownStatus(&uds_download_state, UDS_STATUS_ALLOW_TRANS_DATA);
             }
             else
             {
         ...
         if (resCode != DCM_E_POSITIVERESPONSE)
         {
             ...
             DsdInternal_SetNegResponse(&gMsgContextType,resCode);
...
/* BEGIN_FUNCTION_HDR
********************************************************************************
* Function Name: DspInternal_Uds_TransferData
*                
* Description:   Implementation of Uds servie 0x36
*                             
* Inputs:        None
*                
* Outputs:       None
...
FUNC(void,DCM_PUBLIC_CODE) DspInternal_Uds_TransferData
(
    void
)
{
    ...
    dataSize = gMsgContextType.reqDataLen - 2UL; /*overflow checking is not required.*/
    ...
    do
    {
        ...
        ret = UdsIsValidStatus(uds_download_state,UDS_STATUS_SEGMENT_FIRST_DATA);
        if(BL_ERR_NOT_OK == ret)
        {
            if (gMsgContextType.reqData[ADPT_UDS_36_SERVICE_INDEX_POS] != expendIndex)
            {
                resCode = DCM_E_WRONGBLOCKSEQUENCECOUNTER;
                break;
            }
            /*set first 0x36 service of a download segment.*/
            UdsSetDownStatus(&uds_download_state, UDS_STATUS_SEGMENT_FIRST_DATA);
        }

        /*check whether the SN is valid.*/
        if(gMsgContextType.reqData[ADPT_UDS_36_SERVICE_INDEX_POS] != expendIndex)
        {
            if (gMsgContextType.reqData[ADPT_UDS_36_SERVICE_INDEX_POS] != (expendIndex - 1))
            {
                resCode = DCM_E_WRONGBLOCKSEQUENCECOUNTER;
            }
            else
...
//        DslInternal_RCRResponsePending(FORCERCR_RP);
        ...
        write_result = REMOTEUPDATE_CallbackWriteData(&gMsgContextType.reqData[ADPT_UDS_36_SERVICE_DATA_POS],
        ...
        {
            gMsgContextType.resData[DCM_INDEX_1] = gMsgContextType.reqData[DCM_INDEX_1];
            gMsgContextType.resDataLen = 0x02u;
...
/* BEGIN_FUNCTION_HDR
********************************************************************************
* Function Name: DspInternal_Uds_RequestTransferExit
*                
* Description:   Implementation of Uds servie 0x37
*                             
* Inputs:        None
*                
* Outputs:       None
...
FUNC(void,DCM_PUBLIC_CODE) DspInternal_Uds_RequestTransferExit
(
    void
)
{
    ...
    else
    {
        ...
        gMsgContextType.resDataLen = 0x01u;
    }
        ...
        {         
            ...
            else
            {
                ...
                /* Check par adressing mode */
                    ...
                    && ((gDcmDspWriteMemoryRangeInfo_3D[addresssegment].AddressingMode - gMsgContextType.msgAddInfo.reqType) < 1u))
                {
                    DsdInternal_ProcessingDoneNoResponse();
                }
                else if((gMsgContextType.msgAddInfo.reqType==UDS_PHYSICAL_ON_CAN_RX) 
                    && ((gDcmDspWriteMemoryRangeInfo_3D[addresssegment].AddressingMode&PHYSICAL_REQ_SUPPORTED) == DCM_REQUEST_ADDRESS_NOT_CORRECT))
                {
                    DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_CONDITIONSNOTCORRECT);
                }
                else
           ...
           /*service = gServiceTable_1[sid];*/
            ...
            if(sid < DCM_NUM_OF_SERVICE)
            {
                ...
                else
                {
                    ...
                    if(((sid == 0x0D)&&gMsgContextType.reqDataLen < (uint32)DCM_REQUIRE_DATA_LENGTH_1_BYTE)
                        ||((sid != 0x0D) && gMsgContextType.reqDataLen < (uint32)DCM_REQUIRE_DATA_LENGTH_2_BYTE))  /*wen modify,37服务1字节，其他至少2字节*/
...
)
{
    ...
    {
        /* removed by larry 2013.11.12 QAC */
        /*if((gP2ServerTimer > 0u) || ((gP2ServerTimer == 0u)  
...
{
    if(gSecurityAcessAttempNumber[gCurrentSecurityAccessRequestLevel - 1] != 0u)
    {
        gSecurityAcessAttempNumber[gCurrentSecurityAccessRequestLevel - 1]--;
        if(gSecurityAcessAttempNumber[gCurrentSecurityAccessRequestLevel - 1] == 0u)
        {
            /*gSecurityAcessAttempNumberExceeded[gCurrentSecurityAccessRequestLevel - 1] = 1u;*/
            DsdInternal_SetNegResponse(&gMsgContextType,DCM_E_EXCEEDNUMBEROFATTEMPTS);
...
STATIC FUNC(void,DCM_PRIVATE_CODE) DsdInternal_UdsRepeatServiceCall(void)
{
    if((gDiagState&DIAG_UDS_PROCESSING) == DIAG_UDS_PROCESSING) 
    {
        ...
        if(gFunctionHandler != DCM_NULL) /*Wen Modify*/
        {
            (*gFunctionHandler)();
        }
        else
