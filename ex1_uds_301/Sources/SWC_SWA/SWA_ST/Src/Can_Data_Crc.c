
/* ***************************************************************************/
/*                                 INCLUDE                                   */
/* ***************************************************************************/
#include "Can_Data_Crc.h"

/* ***************************************************************************/
/*                                 GLOBAL VARIABLES                          */
/* ***************************************************************************/
/* ** Crc8H2FTable Definition ***/    
const uint8 Crc8H2FTable[256] = 
{
   
};

/* ***************************************************************************/
/*                         LOCAL PROTOTYPES                                  */
/* ***************************************************************************/


/* ***************************************************************************/
/*                              SOURCE CODE                                  */
/* ***************************************************************************/

uint8 Crc_CalculateCRC8H2F(uint8 *pu8DataBuf,uint16 length)
{
   /*** This is a CRC callculatoin according to AUTOSAR_SWS_CRCLibrary ***/
   uint16 Loc_u16Idx;
   uint8  Loc_u8Data;
   uint8  Loc_u8CrcResult;
  
   Loc_u8CrcResult = 0xFF;
   for(Loc_u16Idx = 0; Loc_u16Idx < length; Loc_u16Idx++)
   {      
      Loc_u8Data      = pu8DataBuf[Loc_u16Idx];
      Loc_u8CrcResult = Crc8H2FTable[Loc_u8CrcResult ^ Loc_u8Data];   
   }
  
   Loc_u8CrcResult ^= 0xFF;
  
   return(Loc_u8CrcResult);
}

